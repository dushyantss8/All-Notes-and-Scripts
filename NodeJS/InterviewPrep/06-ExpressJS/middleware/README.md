...
Middleware forms an ordered pipeline. It can enrich req/res, short-circuit a response, or delegate with next(error).


## Core APIs and concepts

- app.use, next, error middleware, router.use, async wrapper
...
2. **What happens if a middleware sends a response then calls next?**  
...
- [ ] Write authentication and authorization middleware with tests.
