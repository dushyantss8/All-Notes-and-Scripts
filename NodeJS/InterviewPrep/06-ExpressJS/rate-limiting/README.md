...
Rate limiting protects capacity and abusive endpoints. In distributed deployments, use a shared store and identify clients carefully.


## Core APIs and concepts

- express-rate-limit, windowMs, limit, keyGenerator, Redis store, Retry-After
...
1. **Why is an in-memory limiter incorrect across replicas?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **Compare fixed window, sliding window, and token bucket.**  
...
- [ ] Rate limit login by account plus IP without enabling easy denial of service.
