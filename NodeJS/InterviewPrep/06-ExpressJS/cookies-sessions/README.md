...
Cookies store small browser state; sessions commonly store only an opaque identifier while server-side state remains in Redis or a database.


## Core APIs and concepts

- cookie-parser, res.cookie, express-session, secure, httpOnly, sameSite
...
1. **Why should production sessions not use the default MemoryStore?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **Compare cookie-based sessions with bearer tokens.**  
...
- [ ] Implement login, regenerate session, and secure logout.
