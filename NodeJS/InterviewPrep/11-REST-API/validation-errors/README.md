...
Validate params, query, and body before business/database work, then map failures to a stable error contract.

## What to know

- **Schema:** Normalize types and reject unknown fields where the API contract requires it.
- **Error shape:** Return safe code, message, field errors, and request ID; log underlying detail server-side.
- **Middleware:** Centralized error middleware prevents stack traces and inconsistent failures.
