...
Queues buffer asynchronous work and decouple producers from consumers, not business correctness.

## What to know

- **Reliability:** Use durable messages, bounded retries, dead letters, idempotent consumers, correlation IDs.
- **Backpressure:** Monitor depth/age and scale within downstream capacity.

## Flow

```mermaid
flowchart LR
  Producer --> Queue --> Consumer --> SideEffect[database / external API]
```
