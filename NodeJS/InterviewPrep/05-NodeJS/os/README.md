...
os exposes host-level information useful for diagnostics and capacity-aware behavior, not application business logic.


## Core APIs and concepts

- cpus, availableParallelism, totalmem, freemem, platform, homedir, tmpdir, networkInterfaces
...
1. **Why is os.cpus().length not always an appropriate worker count?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **How should containers affect capacity assumptions?**  
...
- [ ] Log a compact diagnostic snapshot without exposing sensitive environment values.
