...
child_process starts independent operating-system processes for commands, services, or isolated workloads.


## Core APIs and concepts

- spawn, exec, execFile, fork, stdin/stdout/stderr, IPC, AbortSignal
...
1. **Why is exec unsafe or unsuitable for unbounded output and untrusted command fragments?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **Compare spawn, exec, execFile, and fork.**  
...
- [ ] Wrap spawn in a Promise with timeout, stderr capture, and exit-code handling.
