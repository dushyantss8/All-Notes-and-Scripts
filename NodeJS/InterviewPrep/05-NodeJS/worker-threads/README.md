...
Workers run JavaScript in separate V8 isolates inside one process and communicate with messages or shared memory.


## Core APIs and concepts

- Worker, parentPort, workerData, MessageChannel, transferList, SharedArrayBuffer, Atomics
...
1. **When do workers help, and when are they worse than async I/O?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **Compare copying, transferring, and sharing ArrayBuffer data.**  
...
- [ ] Create a worker pool for CPU-bound image or hash work.
