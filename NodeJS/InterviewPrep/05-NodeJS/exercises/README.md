...
Practice by predicting scheduling behavior, implementing safe I/O boundaries, and profiling real workloads.


## Core APIs and concepts

- event loop, streams, workers, errors, shutdown
...
1. **How would you test an asynchronous Node utility deterministically?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **Prioritize exercises by risk: shutdown, backpressure, CPU blocking.**  
...
- [ ] Complete the drills below and write down measured outcomes.
