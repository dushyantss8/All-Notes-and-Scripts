...
Cluster starts multiple Node processes that share a server port, improving multi-core utilization and failure isolation.


## Core APIs and concepts

- cluster.isPrimary, fork, workers, IPC messages, schedulingPolicy
...
1. **Contrast cluster with worker_threads and an external process manager.**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **What state-sharing issues occur with cluster workers?**  
...
- [ ] Build a primary process that restarts failed HTTP workers.
