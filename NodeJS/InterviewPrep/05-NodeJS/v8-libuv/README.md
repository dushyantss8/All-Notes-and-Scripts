...
V8 parses, optimizes, and executes JavaScript; libuv supplies the event loop, thread pool, and cross-platform async I/O layer.


## Core APIs and concepts

- V8 garbage collection, JIT, libuv thread pool, UV_THREADPOOL_SIZE, async OS APIs
...
1. **Which Node operations use the libuv thread pool, and which use OS asynchronous facilities?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **Compare fs.readFile, crypto.pbkdf2, and network socket handling.**  
...
- [ ] Measure the effect of four concurrent PBKDF2 calls with different thread-pool sizes.
