...
package.json describes a package's identity, scripts, dependencies, engines, and publication surface; the lockfile captures a reproducible dependency graph.


## Core APIs and concepts

- scripts, dependencies, devDependencies, peerDependencies, optionalDependencies, engines, exports, npm ci
...
1. **Differentiate dependencies, devDependencies, and peerDependencies.**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **Why commit package-lock.json and use npm ci in CI?**  
...
- [ ] Create scripts for lint, test, development, production, and security audit.
