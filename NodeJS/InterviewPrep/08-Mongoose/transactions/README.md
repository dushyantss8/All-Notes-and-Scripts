...
Mongoose sessions wrap MongoDB transactions. Pass the session to every participating operation and keep callbacks retry-safe.


## Core APIs and concepts

- startSession, withTransaction, session, Connection#transaction
...
1. **Why does forgetting session on one query break atomicity?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **How do transaction retries affect side effects?**  
...
- [ ] Create an order and decrement inventory transactionally.
