...
Mongoose validation runs before save and can also run on updates when requested. Database constraints remain the final concurrency boundary.


## Core APIs and concepts

- required, enum, min, validate, ValidationError, runValidators
...
1. **Why are update validators different from document validation?**  
   Start with the invariant or runtime behavior, then state the trade-off and a concrete operational example.
2. **How do you return validation errors to an API client?**  
...
- [ ] Build async and cross-field validators.
