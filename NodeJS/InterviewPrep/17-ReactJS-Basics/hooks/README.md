...
Use `useState` for local UI state, `useEffect` for synchronization with external systems, `useMemo`/`useCallback` only when profiling shows referential churn matters, and custom hooks to share stateful logic. Never call hooks conditionally.
